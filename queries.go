...
func generatePayload(queryName string, variables map[string]interface{}) ([]byte, error) {
	if queryName == "recv" {
	...
	payload := Payload{
		QueryName:  queryName,
		Variables:  variables,
		Extensions: map[string]string{"hash": QUERIES[queryName]},
	}
	return json.Marshal(payload)
}

func generateRecvPayload(variables map[string]interface{}) ([]byte, error) {
	...
	if rand.Float64() > 0.9 {
		payloads = append(payloads, RecvPayload{
			Category: "poe/statsd_event",
			Data: map[string]interface{}{
				"key":        "poe.speed.web_vitals.INP",
				"value":      rand.Intn(26) + 100, // rand.Intn(26) generates a number between 0 and 25
				"category":   "time",
				"path":       "/[handle]",
